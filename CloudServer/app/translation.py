...
class CUDATranslator:
    ...
    def __init__(self, settings: Settings):
        ...
        self._seq2seq = "madlad" in settings.translation_model.casefold()
    ...
    def load(self) -> None:
        ...
        with self._load_lock:
            ...
            model_class = AutoModelForSeq2SeqLM if self._seq2seq else AutoModelForCausalLM
            ...
            if self._seq2seq and self.settings.translation_adapter:
                from peft import PeftModel
                self._model = PeftModel.from_pretrained(
                    self._model, self.settings.translation_adapter
                )
            self._model = self._model.eval()

    def warmup(self) -> None:
        """Pay one-time CUDA graph/kernel costs before accepting live speech."""
        self.translate("Hello", "en", "ko", "__warmup_en_ko__")
        self.translate("안녕하세요", "ko", "en", "__warmup_ko_en__")
        self.reset("__warmup_en_ko__")
        self.reset("__warmup_ko_en__")
    ...
    def translate(
        ...
        if self._seq2seq:
            prefix = {"en": "<2en>", "ko": "<2ko>"}[target_language]
